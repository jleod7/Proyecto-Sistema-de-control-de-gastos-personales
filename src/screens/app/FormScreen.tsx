...
const CATEGORIAS_SUGERIDAS = [
  "Alimentación", "Transporte", "Educación", "Salud",
  "Hogar", "Entretenimiento", "Servicios", "Otros",
];
...
export const FormScreen = ({ route, navigation }: Props) => {
  ...
  const { fotoUri, pickFromCamera, pickFromGallery, clearPhoto } = useCamera();
  const { coordenadas, obteniendo, obtenerUbicacion, limpiarUbicacion } = useLocation();
  ...
  const loadGasto = async (gastoId: string): Promise<void> => {
    try {
      ...
      if (gasto.latitud !== undefined && gasto.longitud !== undefined) {
        setExistingCoordenadas({ latitud: gasto.latitud, longitud: gasto.longitud });
      }
    } catch (error) {
  ...
  const handleInputChange = (key: string, value: string) => {
    ...
    if (key === "fecha" && feriados.length > 0) {
      ...
      if (feriadoEncontrado) {
        setAvisoFeriado(`🎉 ¡Esta fecha es feriado en Ecuador! "${feriadoEncontrado.localName}"`);
      } else {
        setAvisoFeriado("");
      }
    }
        ...
        <View style={formStyles.fotoPicker}>
          {previewFoto ? (
            ...
            <View style={formStyles.fotoPlaceholder}>
              ...
              <Text style={formStyles.fotoPlaceholderText}>Sin foto adjunta</Text>
            </View>
          )}
        </View>
        <View style={formStyles.fotoBtns}>
          <TouchableOpacity style={formStyles.fotoBtnCamera} onPress={pickFromCamera}>
            <Text style={formStyles.fotoBtnText}>📷 Cámara</Text>
          </TouchableOpacity>
          <TouchableOpacity style={formStyles.fotoBtnGallery} onPress={pickFromGallery}>
